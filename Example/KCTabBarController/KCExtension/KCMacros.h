...
/// 用户自定义数据
#define     KCUserDefaults              [NSUserDefaults standardUserDefaults]
/// URL
#define     KCURL(urlString)            [NSURL URLWithString:urlString]
/// 图片
#define     KCImage(imageName)          (imageName ? [UIImage imageNamed:imageName] : nil)
#define     KCPNG(X)                    [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:X ofType:@"png"]]
#define     KCJPG(X)                    [[UIImage alloc] initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:X ofType:@"jpg"]]
/// 字符串
#define     KCNoNilString(str)          (str.length > 0 ? str : @"")
/// 方法名
#define     KCStirngFormSelector(s)     [[NSString alloc] initWithUTF8String:sel_getName(s)]
/// 颜色
...
#pragma mark - # 多线程
#define     KCBackThread(block)         dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), block)
#define     KCMainThread(block)         dispatch_async(dispatch_get_main_queue(), block)
#define     KCMainBarrier(block)        dispatch_barrier_async(dispatch_get_main_queue(), block)
#define     KCMainAfter(x, block)       dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(x * NSEC_PER_SEC)), dispatch_get_main_queue(), block);
...
#pragma mark - # 循环引用消除
    ...
    #endif
#endif
#define     KCWeakSelf(type)            __weak typeof(type) weak##type = type;
#define     KCStrongSelf(type)          __strong typeof(type) strong##type = type;
